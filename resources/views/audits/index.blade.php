...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
    ...
    @forelse($events as $event)
        ...
        <div class="bg-white rounded-3xl premium-shadow {{ $isToday ? 'border-indigo-200 ring-1 ring-indigo-500/10' : 'border-slate-100' }} overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col group relative">
            @if($isToday)
            ...
            <div class="p-8 pb-6 border-b {{ $isToday ? 'border-indigo-100/50' : 'border-slate-100' }} bg-white relative z-10">
                <div class="flex justify-between items-start mb-6">
                    @if($isPast)
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest bg-emerald-50 text-emerald-600 border border-emerald-200">Past</span>
                    @elseif($isToday)
                    ...
                    @endif
                    <span class="text-xs font-bold {{ $isToday ? 'text-rose-600 bg-rose-50 border-rose-100' : 'text-indigo-600 bg-indigo-50 border-indigo-100' }} px-3 py-1.5 rounded-lg border shadow-sm">{{ $eventDate->format('M d, Y') }}</span>
                </div>
